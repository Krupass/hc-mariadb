...
from utils.utils import get_default_mariadb_config_path
from utils.utils import get_default_mariadb_exec_path
...
def parse_args():
    ...
    parser.add_argument("-p", "--path", type=Path, help='Select custom path to MariaDB configuration.',
                        default=get_default_mariadb_config_path())
    ...
    parser.add_argument("--peth", type=Path, help="Select path to mysql.exe", default=get_default_mariadb_exec_path())
...
def generate_database_documentation(dbname, user='root', password='', host='localhost', port='3306',
    ...
    path = path or get_default_mariadb_config_path()
    peth = peth or get_default_mariadb_exec_path()
